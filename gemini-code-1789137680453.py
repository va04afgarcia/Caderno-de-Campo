...
def process_caderno(pu_file, template_file):
    ...
    # Dicionário abrangente e extensível de códigos de culturas do IFAP (com todas as variantes de Olival)
    cult_map = {
        31: '031 - Olival',
        32: '032 - Olival Tradicional',
        33: '033 - Olival Intensivo/Superintensivo',
        34: '034 - Vinha',
        89: '089 - Pousio',
        109: '109 - Amendoal',
        112: '112 - Noz',
        ...
        200: '200 - Tomate Indústria',
        210: '210 - Melão / Melancia',
    ...
    # Mapeamento robusto com fallback descodificado para qualquer cultura desconhecida
    def map_cultura(code):
        if code in cult_map:
            return cult_map[code]
        return f"{code} - [Cultura IFAP Descodificada]"
    ...
    # Mapeamento abrangente de intervenções/medidas PEPAC / Agroambientais
    code_mapping = {
        'C.1.1.1.2': 'UEA', 
        'C.1.1.1.1.2': 'ENR', 
        'C.1.1.8.2': 'AB', 
        'C.1.1.7': 'PRODI',
        'C.1.1.2': 'PRAG',
        'C.1.1.5': 'MANUT_PASTAGEM'
    }
    
    def map_intervention(code):
        if pd.isna(code):
            return None
    ...
    if not df_maa.empty and 'intCodigo' in df_maa.columns:
        df_maa['abbrev'] = df_maa['intCodigo'].apply(map_intervention)
    ...
    def get_esperada(cultura):
        ...
        return "2" # Fallback produtivo genérico
